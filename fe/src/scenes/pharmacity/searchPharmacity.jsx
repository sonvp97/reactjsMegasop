...
function Search() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.get(
        "https://api-gateway.pharmacity.vn/api/product-search?search=" +
          form.search +
          "&size=50"
      );
      ...
      const newData = products.map((item) => {
        return {
          ...
          link: "https://www.pharmacity.vn/" + item.node.slug + ".html",
  ...
  const handleConfirm = async () => {
    try {
      ...
      const response = await axios.post("http://127.0.0.1:8000/linkPharmacity", {
